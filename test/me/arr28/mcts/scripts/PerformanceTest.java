...
import me.arr28.algs.mcts.MCTSTree;
import me.arr28.algs.mcts.ScoreBoard;
import me.arr28.algs.mcts.ScoreBoardFactory;
import me.arr28.algs.mcts.TreeNode;
import me.arr28.algs.mcts.ScoreBoard.PlainScoreBoardFactory;
...
import me.arr28.algs.mcts.policy.rollout.DummyRolloutPolicy;
import me.arr28.algs.mcts.policy.rollout.SimpleRolloutPolicy;
import me.arr28.algs.mcts.policy.tree.NoExpandPolicy;
import me.arr28.algs.mcts.policy.tree.RandomSelectPolicy;
import me.arr28.algs.mcts.policy.tree.SimpleExpandPolicy;
import me.arr28.algs.mcts.policy.tree.SimpleUpdatePolicy;
import me.arr28.algs.mcts.policy.tree.UCB1ApproxSelectPolicy;
import me.arr28.algs.mcts.policy.tree.UCB1SelectPolicy;
...
public class PerformanceTest {
    ...
    public PerformanceTest() {
        ...
        if (lGameStateFactory == null) {
            switch (GAME) {
                case CONNECT4:
                    lGameStateFactory = new C4GameStateFactory();
                    break;
                case CONNECT4FAST:
                    lGameStateFactory = new FastC4GameStateFactory();
                    break;
                case HEX:
                    lGameStateFactory = new HexGameStateFactory();
                    break;
                case BREAKTHROUGH:
                    lGameStateFactory = new BreakthroughGameStateFactory();
                    break;
                default:
                    throw new IllegalStateException("Invalid game selected");
            }
        }
    ...
    private void testPureRolloutSpeed() {
        if (NUM_PURE_ROLLOUTS > 0) {
            ...
            System.out.println(
                    " = " + (long) (NUM_PURE_ROLLOUTS * ((double) 1_000_000_000 / lRolloutTime)) + " rollouts/s");
        }
    }

    private void testFullMCTSSpeed() {
        if (NUM_MCTS_ITERATIONS == 0)
            return;
        ...
        for (int lThreads = 1; lThreads <= MAX_THREADS; lThreads++) {
            ...
            System.out.println(lThreads + " thread(s) performed " + lThisSpeed + " iterations/s ("
    ...
    private long testFullMCTSSpeedNThreads(int xiNumThreads) {
        ...
        for (int lTestRun = 0; lTestRun < NUM_REPEATS; lTestRun++) {
            System.gc();
            MCTSTree lMCTS = new MCTSTree(lSelectPolicy, lExpandPolicy, lRolloutPolicy, lUpdatePolicy,
                    mGameStateFactory, lScoreBoardFactory);
